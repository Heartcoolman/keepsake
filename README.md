...
- **端到端加密落盘**：照片、日记、聊天、人脸特征、人格画像全部密文存储，密钥由用户密码派生（scrypt，参数随记录版本化，登录时自动升级到当前强度）——服务器管理者拿到磁盘也读不到**内容**；忘记密码只能用注册时展示一次的恢复码找回。<br/><sub>边界：为了不解密就能命中人脸/深度缓存，每条记录的照片 SHA-256 以明文保存，因此持有磁盘的人可以验证「某张已知照片是否在库中」，但读不到照片本身或任何文本。时间戳、条目数量等结构字段同样明文。</sub>
...
- scrypt 代价参数按记录版本化（`kdfVersion`）：老账号在下次登录时被透明重新包裹到当前强度，不需要停机迁移
- 家庭共享走密钥包裹（X25519 sealed box），成员退出即轮换家庭密钥并重加密缓存；轮换期间新旧两把密钥都持久化，中断后下次登录自动续跑
- 人脸推理完全在自托管服务器本地进行，任何生物特征数据不出服务器
- 管理台（`/ops`）零用户密钥设计：管理员可管账号、备份、审计，但读不到任何用户内容（[docs/ops.md](docs/ops.md)）
- **不加密的部分**：条目的照片 SHA-256、时间戳、所有权/家庭归属等结构字段以明文保存（启动索引与免解密缓存命中需要）。持有磁盘者可据此确认某张已知照片是否存在、推断使用节奏，但读不到任何内容

> **单进程部署**
> 内存 keyring、限流计数、推理队列都在进程内，没有共享后端。请按单实例部署 —— 起多个副本会导致家庭密钥分裂、限流失效。个人/家庭规模下这是有意的取舍。
